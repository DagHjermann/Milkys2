...
2. Data
    - Last year's data (produced by script 100) - note that most PCBs are lacking  
...
- Could rename them, here we just remove them
```{r}
...
df_duplicates <- dat_new2 %>%
  add_count(STATION_CODE, LATIN_NAME, TISSUE_NAME, SAMPLE_DATE, SAMPLE_NO, PARAM) %>%
  filter(n > 1)
...
### b2. Check legacy data for duplicates   
(11-12 seconds)
```{r}
...
if (nrow(df_duplicates) > 0){
  ...
  stop("Duplicates in the data! Check 'df_duplicates'. (Section 10d)\n")
} else {
  cat("No duplicates found in the data. \n")
}
...
if (nrow(check_unit2) > 0){
  stop(nrow(check_unit2), 
       " param/units have forbidden units and no preferred unit! Fix excel table 'preferred_units'.  (Section 4d)")
} else {
...
if (nrow(check_unit4b) > 0){
  stop(nrow(check_unit4b), 
       " UNIT - preferred unit pairs lack conversion factor! Fix excel table 'preferred_units'. (Section 4d)")
} else {
...
sel <- with(data_updated2, PARAM %in% "EROD" & !TISSUE_NAME %in% "Lever")
data_updated2$TISSUE_NAME[sel] <- "Lever"

message("TISSUE_NAME set to 'Lever' for ", sum(sel), " EROD records")

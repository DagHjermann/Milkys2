...
3. Reformat 2019 data so they conform with the legacy data  
...
filepattern <- "01_df_2020_notstandard_"                       # file name except date and extension
filepattern_with_extension <- "01_df_2020_notstandard_.+.rds"  # file name, rds files only
...
```
### a4. Check PAH metabolites in cod bile      
...
```
### a5. Remove adjusted PAH metabolites (PYR1O, PA1O, BAP3O)  
```{r}
...
if (nrow(df_duplicates) > 0){
  stop("Duplicates in the data! Check 'df_duplicates'. \n")
...
if (nrow(check_unit2) > 0){
  stop(nrow(check_unit2), 
       " param/units have forbidden units and no preferred unit! Fix excel table 'preferred_units'.")
} else {
...
if (nrow(check2) > 0){
  stop("NOTE: Some records have 3-OH-benzo[a]pyren but not BAP3OH. Do not delete all. \n")
}
...
if (sum(abs(check3$Difference_percent) > 5) > 0){
  stop("NOTE: The difference between 3-OH-benzo[a]pyren and BAP3OH exceeds 5%. Check before deletion. \n")
}
...
if (nrow(check2) > 0){
  stop("NOTE: Some records have 1-OH-pyren but not PYR1OH. Do not delete all. \n")
}
...
if (sum(abs(check3$Difference_percent) > 5) > 0){
  stop("NOTE: The difference between 1-OH-pyren and PYR1OH exceeds 5%. Check before deletion. \n")
}
...
if (!exists("df_duplicates_all")){    # recalculate only if it doesn't exist
...
if (nrow(df_duplicates_all) > 0){
  stop("Duplicates in the data! Check 'df_duplicates_all'. \n")
...
### Adding data for VDSI in 36G in 2019 
```{r}
...
sel <- with(data_updated2, PARAM %in% c("EROD","PROTV") & !TISSUE_NAME %in% "Lever")
data_updated2$TISSUE_NAME[sel] <- "Lever"

message("TISSUE_NAME set to 'Lever' for ", sum(sel), " EROD and PROTV records")
...
### Save data from 2015 until now in Excel format

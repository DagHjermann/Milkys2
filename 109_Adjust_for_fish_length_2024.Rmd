...
* Before this script is run, you need to download the labware sample and import data (from Labware/LIMS) 
on your PC using script 801 from project 'Milkys2_pc"    
* **NOTE: The script does currently NOT add length to isotope results - this shuld be fixed** 

## Overview  

- 1. Packages & functions  
- 2. Read data  
    - 2a. Reads 'data_all', which is the most recent 105 file and has LNMEA through 2018  
    - 2b. Length data  
...
# Data on length  
filename <- paste0("Data/150_Fish_individual_data_ELU_", selected_year, ".rds")  #ELU endret
...
data_ind_fish <- readRDS(filename) %>%
  mutate(
    STATION_CODE = case_when(
      ...
      STATION_CODE %in% "43BH" ~ "43B2",                                            # legger til en rettelse her for stasjon 43B
      TRUE ~ STATION_CODE)
  )
...
df_fishsamples <- df_samples %>%
  select(AQUAMONITOR_CODE, TEXT_ID, BIOTA_SAMPLENO, TISSUE, X_BULK_BIO, DESCRIPTION) %>%
  distinct(AQUAMONITOR_CODE, TEXT_ID, TISSUE, BIOTA_SAMPLENO, X_BULK_BIO, DESCRIPTION) %>%
  mutate(
...
if (FALSE){
  ...
  # Check two cod stations
  xtabs(~AQUAMONITOR_CODE + TISSUE, df_samples %>% filter(substr(AQUAMONITOR_CODE,1,1) %in% c("4")))
  xtabs(~AQUAMONITOR_CODE + TISSUE_NAME, df_fishsamples %>% filter(substr(AQUAMONITOR_CODE,1,1) %in% c("4")))
}
```

### c2. Example of number/tissue confusion   
```{r}

if (TRUE){
  
  df_fishsamples %>% 
    filter(AQUAMONITOR_CODE %in% "45B2") %>%
    xtabs(~BIOTA_SAMPLENO + TISSUE_NAME, .)

}

# Example - number 1 is 2 different liver, 4 different muscle    
cat('AQUAMONITOR_CODE %in% "45B2" & BIOTA_SAMPLENO == 1: \n')
df_fishsamples %>% 
  filter(AQUAMONITOR_CODE %in% "45B2" & BIOTA_SAMPLENO == 1) %>%
  xtabs(~DESCRIPTION, .)
cat('\n\n')

# plus that number 101 is 2 more livers    
cat('AQUAMONITOR_CODE %in% "45B2" & BIOTA_SAMPLENO == 101: \n')
df_fishsamples %>% 
  filter(AQUAMONITOR_CODE %in% "45B2" & BIOTA_SAMPLENO == 101) %>%
  xtabs(~DESCRIPTION, .)


```

### c3. Also use labware import file   
- In order to remove siloxans and other confusing stuff from 'df_fishsamples'   
- Creates 'df_fishsamples1'  
```{r}


files <- dir("Input_data", "Labware_import_") %>% rev()

sel <- selected_year %in% as.numeric(substr(files, 16, 19))
...
filename <- files[1] 
df_import <- readRDS(paste0("Input_data/", filename))
# xtabs(~TISSUE, df_samples)
...
if (TRUE){
  ...
  check <- df_import %>% 
    filter(AQUAMONITOR_CODE %in% "43B2") %>%
    mutate(
      Type = case_when(
        grepl("SIA", DESCRIPTION) & grepl("muskel", DESCRIPTION) ~ "Muskel SIA",
        !grepl("SIA", DESCRIPTION) & grepl("muskel", DESCRIPTION) ~ "Muskel",
        !grepl("SIA", DESCRIPTION) & grepl("lever", DESCRIPTION) ~ "Lever",
        TRUE ~ "Annet"))
  ...
  # As above but 30 liver:   
  check %>%
    distinct(Type, TEXT_ID) %>%
    xtabs(~Type, .)
...
# df_import_cod_1: keep only fish (cod)
df_import_cod_1 <- df_import %>%
  left_join(df_samples %>% select(TEXT_ID, TISSUE)) %>% # xtabs(~TISSUE, .)
  filter(TISSUE %in% c("LI-Lever", "MU-Muskel"))

xtabs(~T_ANALYSIS_METHOD, df_import_cod_1)  
# xtabs(~PARAM + T_ANALYSIS_METHOD, df_import_cod_1)  

# df_import_cod_2: drop NILU (i.e. D4,D5,D6 = siloxans)
# - for now, we also drop IFE (isotopes) - but they should be included later!
df_import_cod_2 <- df_import_cod_1 %>%
  filter(!T_ANALYSIS_METHOD %in% c("EKSTERN_NILU", "EKSTERN_IFE"))

df_fishsamples1 <- df_fishsamples %>%
  filter(TEXT_ID %in% unique(df_import_cod_2$TEXT_ID))

```

### c4. New check 
- Check that 'df_fishsamples1' is less confusing than 'df_fishsamples'  
```{r}

# Example - number 1 is just 1 liver and 1 muscle    
cat('AQUAMONITOR_CODE %in% "43B2" & BIOTA_SAMPLENO == 1: \n')
df_fishsamples1 %>% 
  filter(AQUAMONITOR_CODE %in% "43B2" & BIOTA_SAMPLENO == 1) %>%
  xtabs(~DESCRIPTION, .)
cat('\n\n')

# plus that number 101 has now been deleted    
cat('AQUAMONITOR_CODE %in% "43B2" & BIOTA_SAMPLENO == 101: \n')
df_fishsamples1 %>% 
  filter(AQUAMONITOR_CODE %in% "43B2" & BIOTA_SAMPLENO == 101) %>%
  xtabs(~DESCRIPTION, .)

```


### c5. Sample data, special case: galle (bile) at 30B                      #ELU kjører ikke for 2023  
- DROPPED FOR NOW
...
### Add rows for biol. effects in cod                                             #ELU kjører ikke for 2023  
...
# DROPPED FOR NOW                                                                     #ELU kjører ikke denne
...
df_fishsamples_temp <- tidyr::separate_rows(df_fishsamples1, X_BULK_BIO)
...
cat("\n")
message("Number of rows in 'df_fishsamples1': ", nrow(df_fishsamples1))   # 1710
message("Number of rows in 'df_fishsamples2': ", nrow(df_fishsamples2))   # 1710

if (TRUE){
 
  # Show example of a pooled fish sample 
  sel <- grepl(",", df_fishsamples1$X_BULK_BIO, fixed = TRUE)
  df_example <- df_fishsamples1[which(sel)[1],]
...
filenames <- dir("Data", paste0("109_adjusted_data_ELU2_", selected_year, ".+.rds"))          #legger til ELU2 i filnavnet for 2022-dataene, men bare ELU for 2024 (?)
...
if (TRUE){
  ...
  data_lastyear %>% 
    filter(STATION_CODE == "23B" & TISSUE_NAME == "Galle") %>% 
    select(MYEAR, STATION_CODE, TISSUE_NAME, SAMPLE_NO2, PARAM, LNMEA)
  
  data_lastyear %>% 
    filter(STATION_CODE == "30B" & TISSUE_NAME == "Lever" & SAMPLE_NO2 == 8) %>% 
    select(MYEAR, STATION_CODE, TISSUE_NAME, SAMPLE_NO2, PARAM, LNMEA)
...
# OLD: If <1% is at/below zero, we delete the zeros
# param_delete_zeros <- names(percent_zero)[percent_zero < 1]
# NEW: we don't delete any values that are zero
...
param_incl_zero <- names(percent_zero)[percent_zero > 0]
...
#debugonce(adjust_selected)                                                                         #ELU kjører ikke

result <- try(adjust_selected(data_all2, sel, var = "VALUE_WW"))        # adjust_selected()
...
for (i in 1:i_max){
  ...
  # NEW: IS SET INSIDE adjust_selected() INSTEAD 
  #
  ...
  if (enough_data)
    result <- try(adjust_selected(data_all2, sel, var = "VALUE_WW", log = log_transform))        # adjust_selected()
  ...
  # Perform regression
  #
  # debugonce(adjust_selected)
  enough_data <- length(unique(data_all2$MYEAR[sel])) > 1
  if (enough_data)
    result <- try(adjust_selected(data_all2, sel, var = "VALUE_DW", log = log_transform))        # adjust_selected()
  if (class(result) != "try-error" & enough_data){
  ...
  enough_data <- length(unique(data_all2$MYEAR[sel])) > 1
  if (enough_data)
    result <- try(adjust_selected(data_all2, sel, var = "VALUE_FB", log = log_transform))        # adjust_selected()
  if (class(result) != "try-error" & enough_data){
...
data_all2 %>%
  filter(PARAM == param & TISSUE_NAME == "Muskel" & MYEAR >= 2018)  %>%
  group_by(STATION_CODE, MYEAR) %>%
  summarize(Prop_length = mean(!is.na(VALUE_WWa)), .groups = "drop") %>%                                       #ELU test
...
# df %>%
#   filter(PARAM == "CB118" & TISSUE_NAME == "Lever" & MYEAR >= 2013 & !is.na(LNMEA))  %>%
#   group_by(STATION_CODE, MYEAR) %>%
...
if (FALSE){
  ...
  # For manual check  
  data_all2 %>%
    filter(MYEAR == selected_year & STATION_CODE == "30B" & 
...
* Some 2021-data (28A2) are collected in January 2022  

```{r}

sel <- !is.na(data_all2$SAMPLE_DATE) &
  data_all2$SAMPLE_DATE >= ymd_hms("2022-01-01 00:00:00") &
  data_all2$SAMPLE_DATE < ymd_hms("2022-03-01 00:00:00")

xtabs(~SAMPLE_DATE + STATION_CODE, data_all2[sel,])
...
* These shall all be 56A  

```{r}

sel <- with(data_all2, STATION_CODE %in% c("56A","56A2") & MYEAR == 2023)
# xtabs(~PARAM + STATION_CODE, data_all2[sel,])

data_all2$STATION_CODE[sel] <- "56A"
data_all2$STATION_NAME[sel] <- "Kvalnes"
...
filename <- paste0("Data/109_adjusted_data_ELU_", file_date, ".rds")                   #ELU-version for 2023-data

# Backup
if (file.exists(filename)){
  timetxt <- substr(Sys.time(), 1, 16)
  timetxt <- sub(" ", "-", timetxt, fixed = TRUE)
  timetxt <- sub(":", "h", timetxt, fixed = TRUE)
  timetxt
  backup_name <- paste0("Data/109_backup_", file_date, "_", timetxt, ".rds")
...
fn <- paste0("Data/109_adjusted_data_ELU_", file_date, ".csv")                          #ELU-version for 2023-data
readr::write_excel_csv(data_all2, fn, na = "")

```

### e3. ADHOC FIX of Delta123C in 2023  

* Error in LIMS, all data appears to be <20  

```{r}

if (FALSE){
  # For correction after running this script  
  file_date <- "2024-09-25"
  filename <- paste0("Data/109_adjusted_data_ELU_", file_date, ".rds")  # ELU-version 
  # file.copy(filename, "Data/109_adjusted_data_ELU_2024-09-25_ERROR_DELTA13C.rds")
  # file.copy("Data/109_adjusted_data_ELU_2024-09-25_ERROR_DELTA13C.rds", filename)
  data_all2 <- readRDS(filename)
  # data_all2 <- readRDS("Data/109_adjusted_data_ELU_2024-09-25_ERROR_DELTA13C.rds")
  # for 2023-data
}

# Correct data (excel from IFE)
dat_isotopes <- read_csv("Input_data/adhoc/milkys_isotopes_2023.csv")

# Data from Nivadatabase (script 80), this gives us the LIMS ID  
filename_80 <- "80_df_2023_notstandard_2024-09-25_04h12m.rds"
dat_new1 <- readRDS(paste0("Input_data/", filename_80))

# Make lookup file for LIMS numbers (LABWARE_TEXT_ID)
lookup_temporary <- dat_new1 %>% 
  filter(NAME %in% "Delta 13C") %>% # filter(SAMPLE_NO == 1) %>% View("1")
  distinct(SAMPLE_ID, STATION_CODE, SAMPLE_NO, LATIN_NAME, TISSUE_NAME, LABWARE_TEXT_ID) %>%
  arrange(STATION_CODE, LATIN_NAME, TISSUE_NAME, SAMPLE_NO)
lookup_temporary$LABWARE_TEXT_ID_1 <- ""
lookup_temporary$LABWARE_TEXT_ID_2 <- ""
split <- strsplit(lookup_temporary$LABWARE_TEXT_ID, ", ")
sel <- map_int(split, length) == 2
lookup_temporary$LABWARE_TEXT_ID_1[sel] <- map_chr(split[sel], 1)
lookup_temporary$LABWARE_TEXT_ID_2[sel] <- map_chr(split[sel], 2)
nrow(lookup_temporary)
 
# There are several LABWARE_TEXT_ID for each sample, so we need to find the correct manually

# View(lookup_temporary)
# - compare manuall with excel file - conclusion:
# LABWARE_TEXT_ID_2 is the correct for eider duck egg
# LABWARE_TEXT_ID_2 is the correct for eider duck blood
# LABWARE_TEXT_ID_2 is the correct for cod
# LABWARE_TEXT_ID_2 is the correct for blue mussel

# Make lookup file with correct LIMS no
lookup_lims_id <- lookup_temporary %>%
  select(STATION_CODE, SAMPLE_ID, SAMPLE_NO, LATIN_NAME, TISSUE_NAME, LABWARE_TEXT_ID_2) %>%
  rename(LABWARE_TEXT_ID = LABWARE_TEXT_ID_2) %>%
  filter(!is.na(LABWARE_TEXT_ID)) %>%
  arrange(LATIN_NAME, TISSUE_NAME, SAMPLE_NO)
nrow(lookup_lims_id)
# View(lookup_lims_id)

# The data in 109 file - with errors
# Turns aout there are also much too few rows, should be > 300
check <- data_all2 %>% 
  filter(MYEAR %in% 2023 & PARAM == "Delta13C") %>%
  distinct(LATIN_NAME, TISSUE_NAME, SAMPLE_NO2, PARAM, VALUE_WW, FLAG1)
nrow(check)
# ONLY 48 samples! So a lot of samples appear to be missing  
# View(check)

# Check the Delta15N (which always has been correct)
check2 <- data_all2 %>% 
  filter(MYEAR %in% 2023 & PARAM %in% "Delta15N") %>%
  distinct(LATIN_NAME, TISSUE_NAME, SAMPLE_NO2, PARAM, VALUE_WW, FLAG1)
nrow(check2)

# new_data will be added as new rows
# start with the 15N data, and just replace the VALUE_WW numbers    
new_data <- data_all2 %>% 
  filter(MYEAR %in% 2023 & PARAM %in% "Delta15N") %>%
  select(-c(VALUE_DW, VALUE_FB, VALUE_WWa, VALUE_DWa, VALUE_FBa, LABWARE_TEXT_ID)) %>%
  # add LIMS ID
  left_join(
    lookup_lims_id %>% select(STATION_CODE, LATIN_NAME, TISSUE_NAME, SAMPLE_NO, LABWARE_TEXT_ID),
    by = c("STATION_CODE", "LATIN_NAME", "TISSUE_NAME", "SAMPLE_NO2" = "SAMPLE_NO"), 
    relationship = "one-to-one"
  ) %>%
  left_join(
    dat_isotopes %>% select(TEXT_ID, `Delta 13C`),
    by = c("LABWARE_TEXT_ID" = "TEXT_ID"),
    relationship = "one-to-one"
  ) %>%
  # Use this as the new VALUE_WW
  mutate(
    PARAM = "Delta13C",
    VALUE_WW = `Delta 13C`
  ) %>%
...
if (selected_year == 2023){
  ...
  # Remove Delta13C data
  data_all2_corrected_1 <- data_all2 %>% 
    filter(!(MYEAR %in% 2023 & PARAM == "Delta13C"))
  nrow(data_all2)
  nrow(data_all2_corrected_1)
  data_all2_corrected_2 <- data_all2_corrected_1 %>%
    bind_rows(new_data)
  
  nrow(data_all2)
  nrow(data_all2_corrected_1)
  nrow(data_all2_corrected_2)

  file_date <- "2024-09-25"
  filename <- paste0("Data/109_adjusted_data_ELU_", file_date, ".rds")  # ELU-version 
  saveRDS(data_all2_corrected_2, filename)
  filename_csv <- sub("rds$", "csv", filename)
  readr::write_excel_csv(data_all2_corrected_2, filename_csv, na = "")  
  
  data_all2_corrected_2 %>% 
    filter(PARAM == "Delta13C" & VALUE_WW > (-50)) %>% # View()
    ggplot(aes(MYEAR, VALUE_WW)) +
    geom_jitter(width = 0.1) +
    facet_wrap(vars(LATIN_NAME, TISSUE_NAME))
  data_all2 %>% 
    filter(PARAM == "Delta13C" & VALUE_WW > (-50)) %>% # View()
    ggplot(aes(MYEAR, VALUE_WW)) +
    geom_jitter(width = 0.1) +
...
### e3. ADHOC FIX of ALAD and EROD in 2023  

* Error in LIMS (blame DHJ)  
* ALAD wrongly entered as EROD, EROD wrongly entered as PROTV  
* Later updated in Nivabasen using script 'milkys_2023_erod-alad_correction.sql' (on DHJ's pc)  
...
if (FALSE){
  ...
  file_date <- "2024-09-25"
  filename <- paste0("Data/109_adjusted_data_ELU_", file_date, ".rds")  # ELU-version 
  # file.copy(filename, "Data/109_adjusted_data_ELU_2024-09-25_ERROR_EROD.rds")
  data_all2 <- readRDS(filename)
  # data_all2 <- readRDS("Data/109_adjusted_data_ELU_2024-09-25_ERROR_DELTA13C.rds")
  # for 2023-data
}

# for info: the original
# df_raw <- read_excel("Input_data/Biological_effects_cod/BEM-JAMP_2023_ARU.xls", sheet = 1)

data_all2_correct <- data_all2

sel1 <- with(data_all2_correct, MYEAR == 2023 & PARAM == "EROD")
sum(sel1)
sel2 <- with(data_all2_correct, MYEAR == 2023 & PARAM == "PROTV")
sum(sel2)
bind_rows(head(data_all2_correct[sel1,], 1), head(data_all2_correct[sel2,], 1))

data_all2_correct$PARAM[sel1] <- "ALAD"
data_all2_correct$PARAM[sel2] <- "EROD"

data_all2_correct$TISSUE_NAME[sel1] <- "Blod"
data_all2_correct$TISSUE_NAME[sel2] <- "Liver - microsome"

data_all2_correct$UNIT[sel1] <- "mg/mL"
data_all2_correct$UNIT[sel2] <- "pmol/min/mg protein"

data_all2_correct$Substance.Group[sel1] <- "Biological effects: molecular/biochemical/cellular/assays"
data_all2_correct$Substance.Group[sel2] <- "Biological effects: molecular/biochemical/cellular/assays"

bind_rows(
  head(data_all2 [sel1,], 1), head(data_all2 [sel2,], 1),
  head(data_all2_correct[sel1,], 1), head(data_all2_correct[sel2,], 1))

data_all2[sel1,] %>% select(STATION_CODE, SAMPLE_NO2, PARAM, TISSUE_NAME, VALUE_WW, UNIT) %>%
  arrange(STATION_CODE, SAMPLE_NO2)
data_all2[sel2,] %>% select(STATION_CODE, SAMPLE_NO2, PARAM, TISSUE_NAME, VALUE_WW, UNIT) %>%
  arrange(STATION_CODE, SAMPLE_NO2)
data_all2_correct[sel1,] %>% select(STATION_CODE, SAMPLE_NO2, PARAM, TISSUE_NAME, VALUE_WW, UNIT) %>%
  arrange(STATION_CODE, SAMPLE_NO2)
data_all2_correct[sel2,] %>% select(STATION_CODE, SAMPLE_NO2, PARAM, TISSUE_NAME, VALUE_WW, UNIT) %>%
  arrange(STATION_CODE, SAMPLE_NO2)

xtabs(~PARAM, data_all2 %>% filter(PARAM %in% ""))

file_date <- "2024-09-25"
filename <- paste0("Data/109_adjusted_data_ELU_", file_date, ".rds")  # ELU-version 
saveRDS(data_all2_correct, filename)
filename_csv <- sub("rds$", "csv", filename)
readr::write_excel_csv(data_all2_correct, filename_csv, na = "")  

```


### f. Save last years as Excel file  
Needed by Norman and Anders   

#### f1. Pick years  
```{r}
from_year <- max(data_all2$MYEAR) - 4                                             #ELU: kjører denne
...
data_export <- data_export %>%
  safe_left_join(data_trophiclevel %>%                                                          #ELU: kjører denne
...
#### f3. Change tissue names  
```{r}

if (FALSE){                                                                             #ELU: har ikke kjørt denne                                                                     
  ...
  data_export %>% 
    filter(TISSUE_NAME == "WO" & PARAM %in% "RSIP") %>%    # imposex on an individual level
    ggplot(aes(MYEAR, VALUE_WW)) +
    geom_jitter(width = 0.2, height = 0.1) +
    facet_wrap(vars(STATION_CODE))             #(vars(STATION_CODE))
...
data_export <- data_export %>%                                                    #ELU: har kjørt denne
...
fn <- paste0(
  "Data/Milkys_raw_data_ELU_",                                                    #ELU-version
...
#export av trophiclevel til MGR

# fn_tl <- paste0(
#   "Data/Milkys_trophic_level_ELU",file_date,
#   " (script 109).xlsx")
# 
# data_trophiclevel %>%
#   writexl::write_xlsx(fn_tl)

```
## 9. Final check for HG in cod  
```{r}

param <- "HG"

data_all2 %>%
  # Select only stations with data for last year (3 lines)  
  group_by(STATION_CODE) %>% 
  mutate(Data_last_year = sum(MYEAR %in% selected_year)) %>%
  filter(Data_last_year > 0) %>%
  # Select parameter ++
  filter(LATIN_NAME == "Gadus morhua" & MYEAR >= 2000 & PARAM %in% param) %>% # View()
  group_by(STATION_CODE, MYEAR) %>%
  summarise(Fraction_WWa = round(mean(is.finite(VALUE_WWa)/is.finite(VALUE_WW)), 2),
            .groups = "drop") %>%
  ggplot(aes(MYEAR, STATION_CODE, fill = Fraction_WWa)) + 
  geom_tile() +
  labs(title = paste("Fraction of data with length-adjusted values,", param))

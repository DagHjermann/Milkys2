...
dat_new2 <- dat_new2 %>%
  mutate(
    PARAM = case_when(
      ...
      grepl("Kortkjedede (SCCP)", PARAM, fixed = TRUE) ~ "SCCP",
      grepl("Mellomkjedede (MCCP)", PARAM, fixed = TRUE) ~ "MCCP",
      TRUE ~ PARAM)
  )
...
if (nrow(check_unit2) > 0){
  stop(nrow(check_unit2), 
       ...
       "Fix excel table 'Lookup table - preferred parameter units.xlsx' (folder Input_data).\n",
...
for (i in seq_along(sum_parameters)){     # go through numbers 1 to 9
  # We add new rows every time we go through the loop
  dat_new5 <- add_sumparameter_exloq(i, sum_parameters, dat_new5)
  }

...
dat_series_extra <- dat_all_prep3 %>%
...
dat_success <- data.frame(seriesno, jags_finished, ok,   
  ...
  left_join(dat_series_trend %>% 
              select(PARAM, STATION_CODE, TISSUE_NAME, LATIN_NAME, N_years, Years_over_LOQ, k_max),
            by = indexvars)
...
- Extremely similar coe, just removed the 'filter(PARAM == param) ' two places  

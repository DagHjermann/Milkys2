...
library(dplyr)       # mutate, group_by, summarize, case_when, etc....
# library(purrr)
library(tidyr)       # for pivot_wider() and pivot_longer()
library(ggplot2)     # plotting
library(lubridate)   # date/time functions (here, 'year()')
library(readr)       # function read_csv() (improved version of read.csv)
library(knitr)       # function 'kable()' (for printing tables)
...
if (!identical(number_of_measurements[1,], number_of_measurements[2,])){                     
  ...
  cat("\n")
  cat("See 2021 version of script for code for fixing\n")
  fix_SCCP <- TRUE
} else {
  message("There are equally many 'eksl. LOQ' and 'inkl. LOQ' values") 
}
...
# Compute sum parameters (including LOQ values)
...
# Compute sum parameters excluding values below LOQ (without_loq = TRUE)  
for (i in seq_along(sum_parameters)){     # go through numbers 1 to 9
  # We add new rows every time we go through the loop
  dat_new5 <- add_sumparameter(i, sum_parameters, dat_new5, without_loq = TRUE)
  }
...
# Count sum parameters - how many values have we got for each?  
dat_new5 %>%
  filter(PARAM %in% c(names(sum_parameters), 
                      paste0(names(sum_parameters), "_exloq"))
  ) %>%
  count(PARAM) %>%
  kable()
...
dat_new5 %>%
  filter(TISSUE_NAME %in% tissue & BASIS %in% "W",
         PARAM %in% c("BDE6S", "BDE6S_exloq")) %>%
  # group_by(STATION_CODE) %>%
  # mutate(Sample = as.numeric(factor(SAMPLE_NO2))) %>% # select(PARAM, STATION_CODE, SAMPLE_NO2, Sample)
  ggplot(aes(STATION_CODE, VALUE, color = PARAM)) +

...
### a9.1. Check SCCP and MCCP values  

- The data have not been corrected   
    - Milkys2_pc script 802 just gets data from Nivabase  
    - But Appendix 3 in that script shows that 'inkl' values are always numeric and therefore transferred to Nivabase    
- This chunk just confirms that  
...
#### DHJ: Den sjekker om det er like mange SCCP eksl. LOQ som SCCP inkl. LOQ

number_of_measurements <- dat_new1 %>%
  filter(NAME %in% c("SCCP eksl. LOQ", "SCCP inkl. LOQ"),
         STATION_CODE != "19N") %>% # View("SCCP")          # 19N doesn't have 'inkl LOQ'
  xtabs(~NAME + STATION_CODE, .)
number_of_measurements                                             #ELU

if (!identical(number_of_measurements[1,], number_of_measurements[2,])){                     
  cat("\n")
  warning("There are not equally many 'eksl. LOQ' and 'inkl. LOQ' values") 
  cat("\n")
  cat("This will be fixed in the next chunk\n")
  fix_SCCP <- TRUE
}
...
number_of_measurements <- dat_new1 %>%
  filter(NAME %in% c("MCCP eksl. LOQ", "MCCP inkl. LOQ"),
         STATION_CODE != "19N") %>% # View("MCCP")          # 19N doesn't have 'inkl LOQ'
  xtabs(~NAME + STATION_CODE, .)
number_of_measurements                                             #ELU
...
### a9.2 Fix SCCP and MCCP  

- Add the lacking 'SCCP eksl. LOQ' values by   
    - select the rows with 'SCCP inkl. LOQ' for samples that lack eksl. LOQ rows
    - copy those rows to a new data frame  
    - change NAME from 'inkl.' to 'eksl.' and set FLAG1 to < (let VALUE be unchanged)    
- Same for MCCP  
...
if (fix_SCCP){
  ...
  # Select data and count the number of SCCP values per sample (= n)
  # If n = 2, it's OK (there are both 'inkl' and 'eksl' values for that sample)
  # If n = 1, one of the values is lacking (and it's the 'eksl' value)
  data_selected <- dat_new1 %>%
    filter(NAME %in% c("SCCP eksl. LOQ", "SCCP inkl. LOQ"),
           STATION_CODE != "19N") %>%          # 19N doesn't have 'inkl LOQ'
    add_count(STATION_CODE, LATIN_NAME, TISSUE_NAME, SAMPLE_DATE, SAMPLE_NO)
  
  # 1. Make a copy of the data for samples which only have 'inkl' values
  # 2. Change NAME and set FLAG1 to "<"
  # 3. Add these to the original data  
  
  data_to_add <- data_selected %>%
    # Pick the rows with n = 1 
    filter(n == 1 & NAME %in% "SCCP inkl. LOQ") %>%
    # Manipulate the values so this becomes the lacking 'eksl' measurement
    mutate(
      NAME = "SCCP eksl. LOQ",
      FLAG1 = "<") %>%
    # Remove n, we don't need it anymore
    select(-n)
  
  # Add to the data
  n1 <- nrow(dat_new1)
  dat_new1 <- bind_rows(dat_new1, data_to_add)
  n2 <- nrow(dat_new1)
  
  message(n2-n1, " 'SCCP eksl. LOQ' rows added to the data")
...
if (fix_MCCP){
  ...
  # Select data and count the number of MCCP values per sample (= n)
  # If n = 2, it's OK (there are both 'inkl' and 'eksl' values for that sample)
  # If n = 1, one of the values is lacking (and it's the 'eksl' value)
  data_selected <- dat_new1 %>%
    filter(NAME %in% c("MCCP eksl. LOQ", "MCCP inkl. LOQ"),
  ...
  data_to_add <- data_selected %>%
    # Pick the rows with n = 1 
    filter(n == 1 & NAME %in% "MCCP inkl. LOQ") %>%
    # Manipulate the values so this becomes the lacking 'eksl' measurement
    mutate(
      NAME = "MCCP eksl. LOQ",
      FLAG1 = "<") %>%
  ...
  message(n2-n1, " 'MCCP eksl. LOQ' rows added to the data")
...
### a9.3. Check SCCP and MCCP values again  

```{r}

cat("\n===========================================\n")
cat("Final check of SCCP: \n")
cat("===========================================\n")
...
cat("\n")
if (!identical(number_of_measurements[1,], number_of_measurements[2,])){                     
  stop("There are not equally many 'eksl. LOQ' and 'inkl. LOQ' values") 
} else {
  message("SCCP data are now OK\n")
}


cat("\n===========================================\n")
cat("Final check of MCCP: \n")
cat("===========================================\n")
...
if (!identical(number_of_measurements[1,], number_of_measurements[2,])){                     
  ...
  message("MCCP data are now OK\n")
}

#
# MANUAL CHECK for the paranoid 
#
...
if (FALSE){
  ...
  check_MCCP <- dat_new1 %>%
    ...
    mutate(
      ...
      FLAG1 %in% "<" ~ paste(FLAG1, VALUE),
      is.na(FLAG1) ~ as.character(VALUE),
      TRUE ~ "error")
      ) %>%
    select(STATION_CODE, LATIN_NAME, TISSUE_NAME, SAMPLE_DATE, SAMPLE_NO, NAME, VALUE_text) %>%
    tidyr::pivot_wider(names_from = NAME, values_from = VALUE_text)
...
```
### b4. Fix some parameter names   

- Some PFAS (following the OECD list of PFAS abbreviations)   
- Must also be updated in the lookup tables for units   
    - 'Lookup table - preferred parameter units.csv'  
...
data_legacy <- data_legacy %>%
  mutate(
    PARAM = case_when(
      PARAM %in% "PFDODA" ~ "PFDoDA",
      PARAM %in% "Perfluorpentansyre (PFPeA)" ~ "PFPA",
      PARAM %in% "PFPeA" ~ "PFPA",
...
# DROP THIS FOR NOW - leads to duplicates!  

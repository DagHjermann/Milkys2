...
library(extrafont)    # Used by add_trend()
...
```{r}
# Medians
data_med2 <- readRDS(file = "Data/data_medians_2018.rds") %>%
...
```{r}
type <- "line"
# dir.create("Figures_41/2nd edition")
# dir.create("Figures_41/3rd edition")
folder <- "Figures_41/4th edition"
...
# HG blue mussel
#
X <- plot_medians_and_trends(c("HG", "Mytilus edulis", "Whole soft body", "51A",  "WW"), 
                             eqs_type = type, xlim = c(1974, 2019), eqs_x = 1980)
X
```

### b. Figures for Sigurd (15B)  
See mail 26.09.2019   
```{r}
# dir.create("Figures_41/Sigurd_stasjon_15B")
folder <- "Figures_41/Sigurd_stasjon_15B"

data_med2 %>%
  filter(STATION_CODE %in% "15B" & nchar(PARAM) == 2) %>%
  xtabs(~MYEAR + PARAM, .)

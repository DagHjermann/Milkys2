---
title: "140_ICES_submission_2023data"
author: "DHJ"
date: "2024-09-21"
output: 
  html_document:
...
**TODO: check 56A2 / 56A, why do we have both in the data?**

Script for making files in ICES format, for submitting last year's data to ICES  

Source of main data (data_ind2):
- data from   
...
save_folder <- paste0("Files_to_ICES/", selected_year)
save_folder_txt <- paste0("Files_to_ICES/", selected_year, "/Test")
save_folder_rds <- paste0("Files_to_ICES/", selected_year, "/Rdata")

# define function
create_folder_if_needed <- function(folder){
  if (!dir.exists(folder)){
    dir.create(folder)
    message("folder ", sQuote(folder), " created")
  } else {
    message("folder ", sQuote(folder), " already exists")
  }
}

create_folder_if_needed(save_folder)
create_folder_if_needed(save_folder_txt)
create_folder_if_needed(save_folder_rds)
...
# - results in error when used. Tried to reinstall bu failed (bad credentials)
# Instead, defined my own safe_left_join (see 'Functions')  
# UPDATE: this package is now renamed to powerjoin 
...
source("140_ICES_submission_functions.R")
...
# Files 
files <- list_files("Data", pattern = "105_data_with_uncertainty.+.rds")
...
data_ind2 <- data_all %>%
  ...
  # Set 36A to 36A1 and 56A2 to 56A
  mutate(
    STATION_CODE = case_when(
      STATION_CODE == "36A" ~ "36A1",
      STATION_CODE == "56A2" ~ "56A",
      TRUE ~ STATION_CODE)
  )
...
fishdata_fn <- paste0("Data/150_Fish_individual_data_ELU_", selected_year, ".rds")  # assuming this file name format
data_ind_fish <- readRDS(fishdata_fn)
...
### a2-3. Remove stations not to submit  

- Particular for 2022  
- Ranfjord stations (I969, I965) 

```{r}

data_all <- data_all %>%
  filter(!STATION_CODE %in% c("I969", "I965"))
...
data_ind2 <- data_ind2 %>% filter(!STATION_CODE %in% c("I964", "I965", "I969"))

# No eider duck (station 19N) in 2022
if (selected_year == 2022){
  data_ind2 <- data_ind2 %>% filter(!STATION_CODE %in% c("19N"))
}
...
### b1. Get tables submitted to ICES before    

* Used in some places below

```{r}

data_previous <- readRDS("Files_to_ICES/2022/Rdata/842_NIVA2022CF_08.rds")        # HARD-CODED
names(data_previous)

data_previous[["03"]]$STNNO %>% table

str(data_previous[["03"]])
str(data_previous[["04"]])
str(data_previous[["10"]])
lapply(data_previous, names)
...
# rev() sorts files in opposite direction (i.e. with the newest first)
files <- dir("Input_data", "83_Labware_samples_") %>% rev()

sel <- selected_year %in% as.numeric(substr(files, 20, 23))

if (sum(sel)==0){
  stop("No files from year ", selected_year, "found! Get it from script 801 on your pc.")
}

# Read last file (number 1, as they are sorted with the newest first)
filename <- files[1] 
df_samples <- readRDS(paste0("Input_data/", filename))

# table(df_samples$AQUAMONITOR_CODE)
...
df_samples <- df_samples %>%
  ...
  # Set 36A to 36A1
  mutate(
    AQUAMONITOR_CODE = case_when(
      AQUAMONITOR_CODE == "36A" ~ "36A1", 
      TRUE ~ AQUAMONITOR_CODE)
  ) %>%
  # Remove "SIA" lines 
...
df_station <- df_station %>% 
  ungroup() %>%
  left_join(df_smpno, by = "LATIN_NAME") 
...
if (nrow(check) > 0){
  ...
  warning("You must create 'data_04_fish_extra' for the lacking specimens. See next chunk. (Error in section 'Table 04 i1')")
...
if (nrow(check) > 0){
  ...
  if (length(unique(check$STATION_CODE)) > 1){
    stop("This code works only if we need to add specimens for a single station. Otherwise something smarter must be written.")
  }
  ...
  extra_subno_numbers <- max_subno + 1:nrow(check)
...
par_ices <- unique(data_previous[["10"]]$PARAM) %>% sort()
# Add some
...
# xtabs(~MATRX + PARAM, data_previous[["10"]] %>% filter(PARAM %in% c("ALAD", "BAP3OH", "EROD", "PA1OH", "PYR1OH")))
# xtabs(~MUNIT + PARAM, data_previous[["10"]] %>% filter(PARAM %in% c("ALAD", "BAP3OH", "EROD", "PA1OH", "PYR1OH")))
```
...
if (selected_year == 2022){
  # A special case discovered further down in this chunk:
  sel <- with(data_04_fish, STATION_CODE == "36B" & Fish_no == 16); sum(sel)
  data_04_fish$SAMPLE_NO_Muscle[sel] <- 16
}
...
data_10_fish <- data.frame(
  ...
  BASIS = case_when(data_conc_fish$PARAM %in% c("DRYWT%", "FATWT%") ~ "W",
                    TRUE ~ "W"),
...
data_10_bluemussel <- data.frame(
  ...
  BASIS = case_when(data_conc_bluemussel$PARAM %in% "DRYWT%" ~ "W",
                    TRUE ~ "W"),
...
data_10_vdsi <- data.frame(
  ...
  UNIT = df_snail$UNIT,
  BASIS = case_when(df_snail$PARAM %in% c("VDSI", "INTF%") ~ "",
                    TRUE ~ "W"),
...
data_10_eider <- data.frame(
  ...
  BASIS = case_when(data_conc_eider$PARAM %in% "DRYWT%" ~ "W",
                    TRUE ~ "W"),
...
if (selected_year != 2022){
  ...
  # without eider
  data_10 <- rbind(data_10_fish, data_10_bluemussel, data_10_length2, data_10_vdsi)
}
...
# Set BASIS to "" for length,etc. + VDSI
# xtabs(~PARAM, data_10)
sel <- data_10$PARAM %in% c("LNMEA", "WTMEA", "VDSI"); sum(sel)
data_10$BASIS[sel] <- ""
...
sel <- data_10$PARAM == "PYR1OH"; sum(sel)
data_10$DETLI[sel] <- 0.5

sel <- data_10$PARAM == "PA1OH"; sum(sel)
data_10$DETLI[sel] <- 0.5
...
data_21 <- data_previous[["21"]]  # restore from "backup"
data_21_back <- data_21  # make backup

# We use 2018 data instead
data_21 <- data_previous[["21"]] %>%
...
if (FALSE){
  ...
  # Not needed in 2023 - these already exist in table 21
...
param_in_21_but_not_in_10 <- data_previous[["10"]] %>%
...
df_metoa <- readxl::read_excel("Input_data/Lookup_tables/ICES_METOA_codes2.xlsx")
...
### Fix SMPNO in table 03 for 30B (Steilene, Oslofjord)  
```{r}
...
# Find 30B in table 04
sel <- data_03$STNNO == "30B"

if (sum(sel) == 1){
  data_03$SMLNK[sel] <- 9
  cat("SMLNK changed to 9 for", sum(sel), "row of table 03")
} else {
  stop("Either zero or >1 rows of table 04 were found (this error is from the 'Table 20' part")
}
...
# 91;2;19N;+79 0.240;+12 6.600;EST;20210613;;;;19N Breøyane;CEMP;CF;B;T~S

data_91_extra <- tibble(
  RECID = "91",
  CRUIS = 2,                         # 2 for hand-picking
  STNNO = new_stations, 
  LATIT = "+79 0.240",    # ICES format
  LONGI = "+12 6.600",    # ICES format
  POSYS = "EST",
  SDATE = "",   # set later
  STATN = "19N Breøyane",
  MPROG = "CEMP",
  WLTYP = "CF", 
  MSTAT = "B",       # B = background
  PURPM = "T~S"      # purpose: T = time series, S = spatial monitoring  
)
...
if (length(check) > 0){
  ...
  message("Stations in tables 10 and 91 are the same")
}

```

### c4. Check using leaflet map  

```{r}

if (FALSE){
  # Run by marking and Ctrl+Enter, or copy-paste to Console
  library(leaflet)
  leaflet() %>%
    addTiles() %>%
    addMarkers(
      lng = minute2decimal(data_91$LONGI),
      lat = minute2decimal(data_91$LATIT), 
      popup = data_91$STATN) 
}
...
### e1. Fix coordinates of station 76A2  
...
sel <- data_91$STNNO == "20B"
# data_91[sel,]
data_91$STATN[sel] <- "20B Longyearbyen"
...
df_dates <- df_samples %>%
  distinct(SAMPLED_DATE, AQUAMONITOR_CODE) %>%
  rename(SAMPLE_DATE = SAMPLED_DATE, STNNO = AQUAMONITOR_CODE)
...
data_91 <- data_91 %>%
  ...
  # Add SAMPLE_DATE
  left_join(df_dates %>% 
              mutate(SDATE = paste0(substr(SAMPLE_DATE,1,4),
                                    substr(SAMPLE_DATE,6,7),
                                    substr(SAMPLE_DATE,9,10))) %>%
              select(STNNO, SDATE),  
            by = "STNNO")
...
#
check <- data_03 %>%
  safe_left_join(data_20 , by = c("SMLNK"), 
                 na_matches = "never")

if (sum(is.na(check$SLABO)) > 0){
  stop("Table 20 lacks some rows")
}

cat("Checked sampling method table (20) vs sample event table (03). If no error, check has passed.")
...
if (!dir.exists(save_folder)){
  message("Creating folder '", save_folder, "'")
...
print_decimal <- function(string){ 
  str2 <- strsplit(string, ",")[[1]]
  y1 <- strsplit(str2[1], " ")[[1]] %>% as.numeric()
  y <- y1[1] + y1[2]/60
  x1 <- strsplit(str2[2], " ")[[1]] %>% as.numeric()
  x <- x1[1] + x1[2]/60
  paste0(round(y, 5), ",", round(x,5))
}

# ICES position strings getting the warning 'Station position appears to be on land'   
# Run line by line
# For each line, copy-paste into search box of norgeskart.no
"+59 53.737,+05 6.514" %>% print_decimal()
"+69 39.180,+18 58.440" %>% print_decimal()

```


### Check specific lines in data   
ICES DATSU errors typically refers to lines in the text file   
```{r}
...
sel <- data %>% map_lgl(~("SHIPC" %in% colnames(.x)))
names(data)[sel] 

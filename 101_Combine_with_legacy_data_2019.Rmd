...
### a2. Check PCBs  
Find that that all except CB118 are lacking in 2019   
...
### c2. Add missing PCBs for 2019   
All missing except CB118 (see 2-a2)
```{r}

# lookup table for species
df_latin <- dat_new2 %>%
  filter(MYEAR >= 2019) %>%
...
df_extra <- read_excel("Input_data/85_PCBs (except CB118) for Jupyterhub.xlsx")
...
df_extra <- df_extra %>%
  select(-LATIN_NAME) %>%
  left_join(df_latin) %>%
  mutate(PARAM = sub("PCB ", "CB", NAME, fixed = TRUE)) %>%
  mutate(MYEAR = 2019,            # hard-coded
         SAMPLE_NO2 = SAMPLE_NO,
         BASIS = "W")             # hard-coded

nrow(df_extra)  # check that it is the same
...
# Check  
# Also lacking in 2017-18 but that doesn't matter
dat_new2b %>%
...
Note that we use ">= 2019", not equal to
```{r}
...
```{r}
# Some 2020 data...
xtabs(~MYEAR, dat_new3) 

# ...for blue mussel
xtabs(~LATIN_NAME + MYEAR, dat_new3) 

# ...in 26A2
xtabs(~STATION_CODE + MYEAR, dat_new3 %>% filter(LATIN_NAME == "Mytilus edulis")) 
...
sel <- dat_new3$MYEAR %in% 2020
dat_new3$MYEAR[sel] <- 2019
...
dat_new6 <- dat_new6 %>%
  ...
  select(-c(BASIS, VALUE, Preferred_unit, Conversion_factor, N_par, QUANTIFICATION_LIMIT))
...
data_updated %>%
  filter(PARAM %in% "CB_S7" & TISSUE_NAME %in% "Lever" & !is.na(VALUE_WW)) %>%

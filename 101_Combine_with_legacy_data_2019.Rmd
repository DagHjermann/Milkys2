...
- This contains all years, but only 2019 data will be used (some of which are marked 2020)   
- The reformatting is to change the data from 'wide' format (one column per substance) to 'long' format (two columns, 'Substance' and 'Value', plus columns for Flag ("<") and unit)     
```{r}
...
filename <- files[1]
dat_new1 <- AqMexport_read_chemistry_table(filename)

# We save the date part of 'filename' (e.g., '2020-04-23')
# This will be used in part 10, when we save the resulting file
file_date <- substr(filename, 17, 26)    # pick out the part of the text from character no. 17 to no. 26

```

### a2. Check PCBs  
Find that all 'Dutch 7' PCBs, excelpt PCB118, are lacking in 2019   
This is fixed in part 3-c2 below   
```{r}

dat_new1 %>%
  filter(Substance %in% c("CB28","CB52","CB101","CB118","CB138","CB153","CB180") &
           year(CatchDateFirst) >= 2014) %>%
  xtabs(~year(CatchDateFirst) + Substance, .)

if (FALSE){
  
  # Check if they have some other names
  dat_new1 %>%
    filter(grepl("28", Substance) &
             year(CatchDateFirst) >= 2014) %>%
    xtabs(~year(CatchDateFirst) + Substance, .)
...
### a3. Check Fat and dry weight    
Seems ok  
```{r}

dat_new1 %>%
  filter(Substance %in% c("Fett", "TTS")) %>%
  xtabs(~year(CatchDateFirst) + Substance, .)

```

### a4. Check of TBT  
- The code below is only for information, and shows that what is given as "TBT" in Aquamonitor, actually is the atom weight of tin (TBTIN in Access, Tributyltinn (TBT)-Sn in Nivabasen), not the ion weight    
- I.e., TBT in Aquamonitor must be multiplied by 2.44, while keeping the same code (`TBT`). This is done in 3c below.      
...
# Aquamonitor data  
df1 <- dat_new1 %>%
  filter(grepl("TBT", Substance) | grepl("tributyl", Substance, ignore.case = TRUE)) %>%
  filter(!grepl("TTBT", Substance))
df1 %>%
  filter(year(CatchDateFirst) == 2019 & StationCode == station) %>%
  select(StationCode, CatchDateFirst, Substance, Value, Flag, Unit)

# Data from Nivabasen  
# These data were read from PC, directly from the Nivabasen tables  
df2a <- readRDS("Data_Nivabasen/01_df_2019_notstandard_2020-07-03.rds")  

df2b <- df2a %>% 
  filter(grepl("Tributyltinn", NAME) & STATION_CODE == station) %>%    # names()
  select(STATION_CODE, SAMPLE_DATE, NAME, VALUE, FLAG1, UNIT)  
df2b
...
### c. Read 2019 data from NIVAbasen  
Made using script 01 in the 'Milkys' R project(on DHJ's computer as well as on Github/NIVAnorge)   
Note that we can do everything else in the procedure for the big Excel sheet _without_ these data.  
This is used to   
- add lacking parameters (see 3c4) 
- add one station, 26A2 (3c4)
- add variables UNCERTAINTY and QUANTIFICATION LIMIT (needed for submission of data to ICES)     
```{r}

df_nivabase <- readRDS("Input_data/01_df_2019_notstandard_2020-08-05.rds")
...
# USED IN PREVIOUS YEARS (and given in the big excel file)
# Some of these (e.g. DBTIN and TPTIN for ion weight) are illogical but need to be like this in 
#   order to conform with legacy data (data_through_2018)
...
df_extra <- df_extra %>%
  ...
  mutate(MYEAR = year,            # year defined in section 1 of scriot  
...
df_uncert <- df_nivabase2 %>%
  mutate(
    MYEAR = year,            # year defined in section 1 of scriot  
    SAMPLE_NO2 = SAMPLE_NO,
    BASIS = "W") %>%
...
# Aquamonitor data at this stage (dat_new2)    
dat_new2 %>%
...
# If so, add it:
if (nrow(check) == 0){
  dat_new2 <- bind_rows(dat_new2, df_to_add) 
}
...
sel <- with(dat_new2, MYEAR == 2019 & TISSUE_NAME == "Lever" & BASIS == "W" & 
              PARAM == "ZN" & UNIT == "MG_P_KG" & VALUE > 1000)
dat_new2$VALUE[sel] <- dat_new2$VALUE[sel]/1000
...
dat_new3d <- dat_new3c %>%
  bind_rows(data_imposex_vdsi[needed_variables])

cat(nrow(data_imposex_vdsi), "observations added \n")


```


### Change name of data back to 'dat_new3'
```{r}
...
if (nrow(test) > 0){
  cat("WARNING! Preferred_unit and Conversion_factor not found for", 
      nrow(test), "records of the following parameters: \n")
  test %>%
    pull(PARAM) %>%
    unique() %>%
    print()
  cat("Please edit 'Parameter_units.xlsx' \n")
} else {
  cat("Preferred_unit and Conversion_factor found for all parameters. \n")
}

...
* Also happens for 'second last year', see below  
```{r}
...
check_duplicates <- trends_for_excel %>%
...
cat("trends_for_excel - number of lines before fixing:", nrow(trends_for_excel), "\n")

if (nrow(check_duplicates) == 0){
  message("No duplicates to fix")
} else if (check == 1){
  trends_for_excel <- trends_for_excel %>%
    ungroup() %>%
...
cat("\n")
cat("trends_for_excel - number of lines after fixing:", nrow(trends_for_excel), "\n")
...
if (FALSE){
  ...
  check %>%
    select(PARAM, Substance.Group, LATIN_NAME, TISSUE_NAME, STATION_CODE, Basis, 
         Yr_2016, Yr_2017, Yr_2018, Yr_2019, EQS_2019, Yr_2020, EQS_2020, N_string, SD_last, EQS) %>%
    View("check")

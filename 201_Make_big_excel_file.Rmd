...
NEW as of Sept. 2022: download the .rds file to PC, not the csv file. Then convert it to csv using R on your PC.
...
if (sum(is.na(data_med2$STATION_CODE)) > 0){
  stop("NOTE!!!! DATA WITHOUT STATION CODE!")
}
...
message("Available version numbers: ", paste(versions, collapse = ", "))

folders_last_year_latest <- folders_last_year[which.max(versions)]

cat("\n")
message("Picked version: ", max(versions))
message("Picked folder: ", folders_last_year_latest)
...
* This should normally NOT be what we do  

**FROM 126_add_lengthadjusted** 

```{r}

# trends_last_year <- readRDS("Data/125_results_2021_04_output/126_df_trend_wide_2021_updated_01.rds")  
...
# Read EQS limits  
EQS_limits <- read.csv("Input_data/Lookup_tables/Lookup_EQS_limits.csv")
...
### Fix problem of scientific notation in csv
```{r}

# Round p-values

vars <- c("Trend p(long) this year", "Trend p(short) this year",
          "Trend. p(long) last year",  "Trend. p(short) last year")

for (var in vars){
  data_xl_lessthans[[var]] <- round(data_xl_lessthans[[var]], 6)
  }

old_options <- options(scipen = 50)
...
if (fn %in% dir("Big_excel_table")){
  ...
  if (decimal_comma){
    ...
    fn_full_usformat <- sub(".csv", "_dec.csv", fn_full, fixed = TRUE)
    write.csv(data_xl_lessthans, file = fn_full_usformat, quote = FALSE, na = "", row.names = FALSE)
  } else {
  ...
  # Save RDS
  # If csv from Jupyterhub doesn't work download this file instead,
  #   and make csv fil on your computer (see script 'RDS_to_CSV.R' in project 'Milkys2_pc'
  #   folder 'Files_from_Jupyterhub_2021\Big_excel_table')

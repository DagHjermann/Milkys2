...
### Prepare 'df_stat' (station information)  
```{r}
# Add new names
...
after1 <- nrow(data_xl)
if (before != after1)
  message("*************** WARNING! Number of rows changed! **************")
...
data_xl <- left_join(data_xl, df_background, by = c("PARAM", "LATIN_NAME", "TISSUE_NAME", "Basis"))
...
df_ddi <- df_ddi %>%
  safe_left_join(df_ddi_minmax, by = c("STATION_CODE", "LATIN_NAME", "TISSUE_NAME", "PARAM", "Basis"),
                 check = "CV")
...
2. Open the file in Excel in the following way:  
    ...
    * In the next window, showing a preview of the file, choose "65001 Unicode (UTF-8)"  
...
8. Go to Developer:Macros (or View:Macros) and run these macros:

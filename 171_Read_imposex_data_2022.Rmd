...
library(stringr)
# library(ggplot2)
# library(niRvana) # Only used for reading from Nivabasen Orracle database (on a pc)
...
# Only used for writing to Nivabasen Orracle database (on a pc)
...
folder_vdsi <- "Input_data/opparbeiding_biota/Snegl_2022/"
...
#
# dir(folder_vdsi)
fn <- "71G_fugløy_intersex_LITTORINA_2022.xlsx"
...
### Change 277G to 277G2   

- which is the code currenølty used 

```{r}

sel <- dat$STATION_CODE %in% "227G"
sum(sel)
dat$STATION_CODE[sel] <- "227G2"
...
### For copying to code in part 2 (definition of 'stations')

```{r}

# For the code in part 2
# dput(unique(dat$STATION_CODE))
...
- The rest of this script *cannot be run on Jupyterhub*    
- It is for adding the Imposex data tp NIVA's Oracle database  
- Must be run with connection to NIVA's Oracle database (on a pc)     

### a. Get samples in Labware file     
```{r}

#
# COPY-PASTE THE CONTENTS OF THIS CHUNK TO RSTUDIO ON YOUR PC
#
...
selected_year <- 2022
stations <- c("11G", "131G", "15G", "227G2", "22G", "36G", "71G", "76G", "98G")

dat <- structure(list(STATION_CODE = c("11G", "131G", "15G", "227G2", 
"22G", "36G", "71G", "76G", "98G"), PARAM = c("VDSI", "VDSI", 
"VDSI", "VDSI", "VDSI", "VDSI", "Intersex", "VDSI", "VDSI"), 
    LATIN_NAME = c("Nucella lapillus", "Nucella lapillus", "Nucella lapillus", 
    "Nucella lapillus", "Nucella lapillus", "Nucella lapillus", 
    "Littorina littorea", "Nucella lapillus", "Nucella lapillus"
    ), VALUE = c(0, 0, 0, 0, 0, 0, 0, 0, 0), MYEAR = c(2022, 
    2022, 2022, 2022, 2022, 2022, 2022, 2022, 2022), TISSUE_NAME = c("Whole soft body", 
    "Whole soft body", "Whole soft body", "Whole soft body", 
    "Whole soft body", "Whole soft body", "Whole soft body", 
    "Whole soft body", "Whole soft body")), row.names = c(NA, 
...
df_samples <- bind_rows(df_labware_01, df_labware_02) %>%
  filter(AQUAMONITOR_CODE %in% stations)

```

### b. Check that we have all stations  
...
# If there are two records of 71G, both with same date

...
**NOTE: SOME (POSSIBLE?) SYNONYMS
* Nonaklor, trans- = Nonaklor   - see 'Lookup table - substance groups.csv'
* Oktaklorstyren (OCS) = OCS    - see 'Lookup table - substance groups.csv' 
* Endosulfan, alfa-  ???
* epsilon-HCH*  ???

**Combine legacy data (data until the year before last year) with new data (downloaded in script 100)**   

* Ran after 050 and 051 (which add imposex and biol. effect data to Nivabasen) and 080 (which downloads data from Nivabasen)  
    - note: this replaces the old routine of running script 802 on your own PC  
* script 080 creates a file in the folder 'Input_data'   
* Overview (more detailed overview below)
    - part 1: read the data from the last year (for 2023, also eider duck 2022 is added)  
    - part 2-8: format/check the last year's data    
    - part 9: add last year's data to legacy data (from running script 101 last year)  
    - part 10-13: checking/correcting the entire data set  
...
if (nrow(dat_plot) > 0){
  ...
  message("No stations contains both versions of synonymous PAH metabolite names")
}
...
if (!identical(check[1,], check[2,])){   
  ...
  message("All OK - there are equally many 'eksl. LOQ' and 'inkl. LOQ' values for all stations")
}                                              
...
legacy_data_file_number <- 1
legacy_data_file <- files[legacy_data_file_number]

data_legacy <- readRDS(paste0("Data/", legacy_data_file)) %>%
...
check <- data_legacy %>%
  filter(MYEAR %in% 2015:(lastyear-1)) %>% 
...
*2023: Skips this here - must add sums for the entire dat set (all years)*
```{r}
...
dat_new6 <- dat_new6 %>%
  ...
  select(-c(BASIS, VALUE, Preferred_unit, Conversion_factor))
...
data_updated <- bind_rows(
  data_legacy, 
  dat_new6 %>% select(-c(SAMPLE_NO, REPNO, REMARK_sample, 
...
df_duplicates <- data_updated %>%
  add_count(STATION_CODE, LATIN_NAME, TISSUE_NAME, MYEAR, SAMPLE_NO2, PARAM) %>%
  filter(n > 1)

if (nrow(df_duplicates) > 0){
  stop("Duplicates in the data! Check 'df_duplicates'. (Section 9) \n")
  xtabs(~PARAM, df_duplicates) %>% print()
  xtabs(~MYEAR, df_duplicates) %>% print()
} else {
  cat("No duplicates found in the data. \n")

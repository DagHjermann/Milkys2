...
dat_eider_2022 <- read_csv(
  ...
  # Keep rows with eider duck only (station 19N)  
  filter(
    STATION_CODE == "19N") %>%
  # Remove rows with isotopes - they were analysed last year so they are already in the legacy data  
  filter(
    !NAME %in% c("% C", "% N", "W% C/N", "Delta 13C", "Delta 15N")  
  ) %>%
  # HARD-CODED: this is year 2022
  mutate(
    MYEAR = 2022)

...
# Use this for adding 'VDSI/Intersex' to existing trends
#
#   df_series <- df_series %>% 
#     filter(PARAM %in% "VDSI/Intersex" & Basis %in% "WW") 
#   } 
#
# Used for adding 'TPTIN' to existing trends
#
#  df_series <- df_series %>% 
...
if (add_extra_series){
  ...
  # Code below: Filter time series already existing in the data
  #   NOTE: This will include entirely new series, but not "old" series which have gotten an extra year  
  
  # Find newest existing time trend data
...
#o#o#o#o#o#o Calculation procedure #o#o#o#o#o#o
t0 <- Sys.time()                                                            # Start time for procedure
zz <- file("Messages from script 120 (can be deleted).txt", open = "wt")    # Makes file for writing error messages

sink(zz, type = "message")             # Redirect error/warning messages - will be sent to the temp.txt file 
result_list_10yr <- args %>% pmap(model_from_medians_stat_s, yrs = 2010:2019, data_medians = data_med)  # ca 15-20 minutes
result_list_long <- args %>% pmap(model_from_medians_stat_s, yrs = 1980:2019, data_medians = data_med)  # ca 15-20 minutes
sink()                                 # Turn of redirection of messages   

Sys.time() - t0  # Print time used - 36.55274 minutes last time  

#o#o#o#o#o#o Calculation procedure ends #o#o#o#o#o#o

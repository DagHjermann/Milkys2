...
lastyear <- 2018   # The series must last at least until this year    
...
if (add_extra_series){
  ...
  if (FALSE)
    df_series <- df_series %>% filter(STATION_CODE %in% "19N" & TISSUE_NAME %in% "Blod" & grepl("HBCD", PARAM))

  # Example 4: Unadjusted ('OH') PAH metabolites 
  if (TRUE)
    df_series <- df_series %>% filter(PARAM %in% c("PYR1OH", "BAP3OH", "PA1OH") & Basis %in% "WW")

...
* This script can be run in two ways:  
    - Interactively - run chunk by chunk (by clicking the little arrow in the corner of the chunk, or Shift+Ctrl+Enter and Shift-Alt-N)  
    - Or by using "Knit with Parameters..." from the menu (pull-down menu from the Knit button)  
...
dat_last_year_pooled <- dat_1 %>%
  filter(MYEAR == measurement_year) %>%
  count(SAMPLE_ID, name = "Pooled_n") 

if (download_and_save){
  
  dat_last_year <- dat_last_year_1 %>%
    left_join(dat_last_year_pooled, by = "SAMPLE_ID") %>%
    collect()
  now <- lubridate::now()
...
if (download_and_save){
  ...
  message("Data saved as", sQuote(fn_csv), "(and corresponding .rds file)")
...
dat_last_year_1 <- dat_2 %>%
  filter(MYEAR == 2023 & STATION_CODE == "30B")

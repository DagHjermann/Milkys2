...
caption_text <- get_caption_text(lookup_ref_value, ref_type1, ref_type2, ref_basis1, ref_basis2, unit) 

#  "Reference value for colors: {ref_type1} = {ref_value1_txt} d.w. (OSPAR species-specific conversion factors used). All 
# data are dog whelk except 71G from 2015 onwards, which are common periwinkle.")
...
params <- "VDSI"
plot_title <- "VDSI (Vas Deference Sequenc Index)" # ; mean imposex stage measured on a 0-6 scale
...
params <- "MBT"
plot_title <- "MBT (monobutyltin)" # ; mean imposex stage measured on a 0-6 scale
...
# caption_text <- glue(
#   "Reference value: BAC = {lookup_ref_value$BAC_dw[1]} d.w. (OSPAR species-specific conversion factors used). All data are dog whelk except 71G from 2015 onwards, which are common periwinkle.")
...
params <- "DBT"
plot_title <- "DBT (tributyltinn, kation weight)"
...
lookup_proref <- read.csv("../Input_data/Lookup_tables/Lookup_proref.csv")
# proref = 1.200000 for DBTIN in Nucella
# conversion factor = 1.96 (from "C:\Data\seksjon 212\AMAP assessments\functions\information functions 2_4.R")
# 1.2*1.96 =  3.2264

lookup_ref_value <- tribble(
  ~LATIN_NAME, ~PROREF,    
  "Nucella lapillus", 1.2,  
  "Littorina littorea", 1.2,  
)
ref_type <- list("PROREF", NULL)
ref_basis <- list("ww", NULL)
...
ref_value1_txt <- paste(unique(lookup_ref_value$Ref_value1), collapse = ", ")
caption_text1 <- glue("All data are given in {unit} w.w.")
if (length(unique(lookup_ref_value$Ref_value1)) > 1){
  caption_text2 <- with(lookup_ref_value, glue("{ref_type1}: {Ref_value1} {unit} {ref_basis1} ({LATIN_NAME})"))
} else {
 caption_text2 <- glue("{ref_type1}: {unique(lookup_ref_value$Ref_value1)} {unit} {ref_basis1} (all species)")
}

...
# NOTE:
# Parameters "SCCP" and "MCCP" until 2014 has been checked and found to be "eksl. LOQ"
...
# HARD-CODED PART for this notebook: Fixing 2021 data
...
# For adding "incl LOQ" value to tooltip
...
# For manual check
# dat_2021 %>% 
#   select(STATION_CODE, LATIN_NAME, TISSUE_NAME, SAMPLE_NO, NAME, VALUE, VALUE_incl) %>% 
#   View("2021")

# Code adapted from script 110, section 3    
dat_2021_med <- dat_2021 %>%
  rename(VALUE_WW = VALUE) %>%        # note: assume w.w.
  group_by(PARAM = NAME, STATION_CODE, LATIN_NAME, TISSUE_NAME) %>%
  summarise(
    N_median = n(), 
...
# NOTE:
dat_2021_med$Over_LOQ <- dat_2021_med$N_median

# NOTE:
sel <- dat_2021_med$STATION_CODE %in% "36A1"; sum(sel)
dat_2021_med$STATION_CODE[sel] <- "36A"


message("nrow(dat_medians_sel_1): ", nrow(dat_medians_sel_1))
dat_medians_sel_1 <- dat_medians_sel_1 %>%
  filter(!MYEAR %in% 2021:2022) %>%
  bind_rows(dat_2021_med)
message("nrow(dat_medians_sel_1): ", nrow(dat_medians_sel_1))
  ...
  data_tiles[[i]] <- data_tiles[[i]] %>%
    mutate(
      Tooltip_txt = case_when(
        MYEAR == 2021 & !LATIN_NAME %in% "Somateria mollissima" ~ paste0(Tooltip_txt, "<br>Incl. LOQ: ", Value_incl, " ", unit),
        TRUE ~ Tooltip_txt
      )
    )
}

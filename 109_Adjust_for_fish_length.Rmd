...
sel <- selected_year %in% as.numeric(str_sub(files, 17, 20))
...
df_lnmea_previous <- data_all_previous %>%
  filter(!is.na(LNMEA) & MYEAR == (selected_year-1)) %>%
  group_by(MYEAR, STATION_CODE, TISSUE_NAME, SAMPLE_NO2) %>%
  summarise(
...
data_all2 <- bind_rows(
  data_all %>% filter(MYEAR < (selected_year-1)),
...
for (i in 1:i_max){
  ...
  # Printout  
  with(data_lengthreg, cat(PARAM[i], LATIN_NAME[i], TISSUE_NAME[i], STATION_CODE[i], "\n"))

...
filename <- "Data/109_adjusted_data_2020-08-05.rds"
data_all_2019 <- readRDS(filename)

df_lnmea_2019 <- data_all_2019 %>%
  filter(!is.na(LNMEA) & MYEAR == 2019) %>%
  group_by(MYEAR, STATION_CODE, TISSUE_NAME, SAMPLE_NO2) %>%
  summarise(
...
if (nrow(check) > 0){
  ...
  df_lnmea_2019 <- df_lnmea_2019 %>%
    select(-LNMEA_diff)
}

```

### Make df_lnmea_since_2019  
```{r}

df_lnmea_since_2019 <- bind_rows(
  df_lnmea_2019, df_lnmea
)
...
message("nrow(data_all), since 2019: ", nrow(data_all %>% filter(MYEAR >= 2019)))  # 752

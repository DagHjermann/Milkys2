...
# Special case 1: Combo plot for HG cod/duck with WW tiles + WWa trends

# 1. Run HG with WW for everything down to and including '04_medians_plot', then:
#   gg_Hg1 <- gg_tile_list[[1]] + labs(title = "Hg in cod and eider duck (trends are length-adjusted)")
# 2. Run HG with WWa for everything before '06_combine_plots', then:
#    gg_tile_list[[1]] <- gg_Hg1
#    speciesgroup <- 1
#    ggcomb_list <- list()
# - Then run inside of loop in '06_combine_plots', but stop before "ggsave", then:
#    filename <- "Figures/431/HG-WW-WWa_combo_1.png"
# - Then run ggsave part in the same chunk
...
if (params$param %in% "HG" & params$basis_fish %in% "WWa"){
...
for (speciesgroup in 1:2){
  ...
  if (nrow(dat_medians_list[[speciesgroup]]) > 0){
    ...
    if (params$param %in% "HG" & speciesgroup == 1 & params$basis_fish == "WW"){
      plot_title <- paste0(dat_medians$Parameter.Name[1], " (not length-adjusted) in ", speciesgroup_txt[[speciesgroup]])
    } else if (params$param %in% "HG" & speciesgroup == 1 & params$basis_fish == "WWa"){
...
for (speciesgroup in speciesgroups){
  ...
  if (params$param %in% "HG" & speciesgroup == 1){
    filename <- glue::glue("Figures/431/{params$param}-{params$basis_fish}_combo_{speciesgroup}.png")
  } else {
    filename <- glue::glue("Figures/431/{params$param}_combo_{speciesgroup}.png")
  }
...
if (params$param == "HG"){
  dat_raw1 <- get_data_raw(params$param, "Gadus morhua", "Muskel", "WWa", include_year=current_year)  
} else {
  dat_raw1 <- get_data_raw(params$param, "Gadus morhua", "Lever", "WW", include_year=current_year)  
}
...
if (params$eqsplot == TRUE){
  ...
  for (speciesgroup in speciesgroups){
    ...
    # Set name of figure file (mercury in cod+birds is treated as a special case)
    if (params$param %in% "HG" & speciesgroup == 1){
      filename <- glue::glue("Figures/431/{params$param}-{params$basis_fish}_eqsratio_ord_{speciesgroup}.png")
    } else {
...
for (speciesgroup in speciesgroups){
  ...
  # Set name of figure file (mercury in cod+birds is treated as a special case)
  if (params$param %in% "HG" & speciesgroup == 1){
    filename <- glue::glue("Figures/431/{params$param}-{params$basis_fish}_prorefratio_ord_{speciesgroup}.png")
  } else {

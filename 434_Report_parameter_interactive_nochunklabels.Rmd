...
for (speciesgroup in speciesgroups){
  ...
  if (space_bottom[speciesgroup] > 0){
    nrow = 2
    rel_heights <- c(3, space_bottom[speciesgroup])
    plotlist <- c(plotlist, NA, NA, NA)
  } else {
    nrow = 1
    rel_heights <- 3
  }
  ...
  # if (params$basis_trends == "WWa" & speciesgroup == 1){
  if (params$param == "HG" & speciesgroup == 1){
    ggcomb_list[[1]] <- ggdraw(ggcomb_list[[1]]) +
      draw_label("* Trends are based on length-adjusted concentrations",
                 x = 0.98, y = 0.02, size = 10, hjust = 1, vjust = 0)
  }

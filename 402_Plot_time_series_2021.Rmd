...
dat_all_prep3 <- bind_rows(
  dat_all_prep3 %>%
    filter(PARAM != "CB118") %>%
    left_join(dat_eqs %>% filter(PARAM != "CB118") %>% select(-LATIN_NAME), by = c("PARAM", "Basis")),
  dat_all_prep3 %>%
    filter(PARAM == "CB118") %>%
    left_join(dat_eqs %>% filter(PARAM == "CB118"), by = c("PARAM", "Basis", "LATIN_NAME"))
) %>%
left_join(dat_proref, by = c("PARAM", "LATIN_NAME", "TISSUE_NAME", "Basis"))
...
```{r test_code, echo=FALSE}

if (FALSE){
  # debugonce(plot_timeseries)
  # debugonce(get_seriesno)
  debugonce(plot_timeseries_seriesno)
  # debugonce(get_unit_text)
  plot_timeseries("HG", "30B",
                            basis = "WWa",
                              y_scale = "ordinary",
                              ymax_perc = 100,
                              xmin_rel = 0,
